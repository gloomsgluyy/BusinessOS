...
| Document Drive | New read-only Document Drive module aggregates Forecast Sales, Shipment, and Daily Delivery documents with search/source/group filters, summary cards, download/open actions, dedicated `document_drive` permission, document-only navbar/server route guard for public users, and Drive-owned file proxy. Critical shipment documents remain hidden from non-executive roles. | Needs production role provisioning and optional folder-style hierarchy if users want a true file-manager UI later. | 92% |
