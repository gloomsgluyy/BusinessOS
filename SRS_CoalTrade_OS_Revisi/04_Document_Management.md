...
## 11. Public Document Drive

### FR-DOC-DRIVE-001 Public Read-Only Access

Document Drive harus dapat dibuka tanpa login untuk kebutuhan external/public document lookup.

Rules:

- URL `/document-drive` dapat diakses tanpa session.
- API read-only `/api/document-drive` dan file proxy `/api/document-drive/files/*` dapat diakses tanpa session untuk dokumen non-critical.
- User tanpa login hanya melihat Document Drive; modul lain tetap terkunci oleh login.
- Critical document tetap tidak boleh tampil/download untuk public user.
- Public user tidak boleh upload, edit, delete, atau membuka source module route.

### FR-DOC-DRIVE-002 Included Document Sources

Document Drive minimal menampilkan:

- Forecast Sales documents,
- Shipment required/additional documents,
- Domestic handover evidence,
- generated Shipping Instruction per shipment.

Generated SI bukan upload manual, tetapi tetap harus searchable dan downloadable dari Document Drive melalui generated PDF proxy.

### FR-DOC-DRIVE-003 Naming and Search

Document naming di Document Drive harus jelas untuk pencarian operasional.

Display title minimal menggabungkan:

- source module,
- Forecast Sales / shipment / MV / nomination owner,
- document type or requirement label,
- buyer if available,
- SI number/version for Shipping Instruction.

Contoh:

- `MV ABC - Shipping Instruction 018 SI-SUPPLIER/V/2026 - v1 - Buyer XYZ`
- `Forecast Sales ABC - Laporan Hasil Verifikasi - Buyer XYZ`
- `TB ABC 107 - Bill of Lading - Buyer XYZ - required`

Search harus mencakup owner, buyer, document type, title, file name, uploader, dan SI number.

### FR-DOC-DRIVE-004 Performance

Document Drive tidak boleh menjalankan schema mutation seperti `ALTER TABLE` pada request listing public.

Acceptance:

- Listing memakai select metadata saja, bukan file bytes.
- Limit default dibatasi.
- File bytes/PDF generation hanya terjadi saat user klik open/download.
- Loading state harus jelas, tetapi listing normal tidak boleh terasa menggantung untuk data sedang.
